  ...
  useEffect(() => {
    ...
    socket.on("privateMessage", (message) => {
      ...
      if (message && message.senderName && message.receiverName) {
        ...
        if (
          message.senderName !== user.username &&
          !onlineUsers.includes(message.senderName)
        ) {
    ...
    const fetchInitialData = async () => {
      try {
        ...
        if (messagesRes.ok) {
          ...
          const private_msgs = allMessages.filter(
            (msg) =>
              msg.receiverName &&
              (msg.senderName === user.username ||
                msg.receiverName === user.username),
          );
          ...
          private_msgs.forEach((msg) => {
            const chatPartner =
              msg.senderName === user.username
                ? msg.receiverName
                : msg.senderName;
            privateMessagesMap[chatPartner] =
              privateMessagesMap[chatPartner] || [];
    ...
    try {
      ...
      if (!previousUsers.includes(user.username)) {
        ...
        localStorage.setItem(
          "chatAppPreviousUsers",
          JSON.stringify(newPreviousUsers),
        );
            ...
            <div className="message-input">
              ...
              <button onClick={sendMessage}>
                ...
                >
                  ...
                  <path
                    d="M3.4 20.4L20.85 12.92c.8-.33.8-1.5 0-1.83L3.4 3.6c-.5-.2-1.03.08-1.2.6L1.1 9.8c-.1.36.05.74.34.9l5.3 2.3c.28.12.28.52 0 .64l-5.3 2.3c-.29.15-.44.53-.34.9l1.1 5.6c.17.5.7.8 1.2.6z"
                    fill="currentColor"
                  />
                </svg>
              </button>
